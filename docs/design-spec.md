> **Note (Spotify-FY): Graceselle spec below is deprecated — this repo is Spotify-FY (Flutter + Node).** The remainder of this file is preserved for history; see `docs/build-spec.md` + `README.md` for the active spec.
> Spotify-FY uses a separate M5 QA spec — this file is not authoritative for UI any more.
>
> ## Spotify-FY TTL rationale: 5h cache < 6h expiry, android first strategy
> - Audio URLs are signed `googlevideo.com` links expiring ~6h. We cache **5h** (`AUDIO_CACHE_TTL_HOURS=5` default, `Song.isAudioCacheValid()` checks `<5h`) so stale URLs are never served.
> - `audioService.js` probes strategies **android → ios → mweb → default → tv_embedded → web_embedded**; android first because it succeeds in ~6s on Render datacenter IPs where `default` times out after 25s. Winner is remembered via `preferredStrategy`.
> - `Song.audioUrlCached`/`audioExtractedAt` + Redis `audio:<videoId>` share the same 5h TTL; Mongo persistence is best-effort (playback still works if DB unreachable).
> - `FRONTEND_URL` CORS supports comma-separated list; `android/app/google-services.json` is gitignored (add local Firebase config).
